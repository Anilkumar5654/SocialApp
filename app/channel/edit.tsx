...
export default function EditChannelScreen() {
  ...
  const { data: channelData, isLoading, isError } = useQuery<ChannelData>({
    queryKey: ['my-channel-profile', currentUserId],
    queryFn: async () => {
        const checkResponse: any = await api.channels.checkUserChannel(currentUserId || '');
        const channelId = checkResponse?.channel?.id;

        if (channelId) {
            const result = await api.channels.getChannel(channelId);
            return result?.channel as ChannelData;
        }
  ...
  const initialHandle = channelData?.handle;

  // --- HANDLE RESTRICTION LOGIC ---
  const timeSinceLastUpdate = useMemo(() => {
    if (!channelData?.last_handle_update) return RESTRICTION_MILLIS;
    
    const lastUpdateDate = new Date(channelData.last_handle_update).getTime();
    const timeElapsed = Date.now() - lastUpdateDate;
    return timeElapsed;
  }, [channelData?.last_handle_update]);
  ...
  useEffect(() => {
    if (channelData) {
      ...
      setChannelHandle(channelData.handle || '');
      setChannelBio(channelData.bio || '');
      setChannelAbout(channelData.about_text || '');
    }
  }, [channelData]);
  ...
  const updateChannelMutation = useMutation({
    ...
    onSuccess: (response: any) => {
      ...
      if (response.avatar_path && channelData) channelData.avatar = response.avatar_path;
      if (response.cover_path && channelData) channelData.cover_photo = response.cover_path;

      queryClient.invalidateQueries({ queryKey: ['my-channel-profile', currentUserId] });
      queryClient.invalidateQueries({ queryKey: ['channel-profile', channelData?.id] });
  ...
  const handleSave = () => {
    ...
    formData.append('channel_id', channelData.id);
    ...
    if (newAvatarUri) {
        ...
        formData.append('avatar', {
            uri: newAvatarUri,
            name: `avatar_${channelData.id}.jpg`,
            type: 'image/jpeg',
        } as any);
    }
    if (newCoverUri) {
        formData.append('cover_photo', {
            uri: newCoverUri,
            name: `cover_${channelData.id}.jpg`,
            type: 'image/jpeg',
        } as any);
    }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          ...
          >
            <Text style={styles.retryButtonText}>Go to Creator Studio</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
  ...
  const currentAvatar = newAvatarUri || getMediaUri(channelData?.avatar);
  const currentCover = newCoverUri || getMediaUri(channelData?.cover_photo);
